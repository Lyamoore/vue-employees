...
const pasDtRules = [
  ...
  (value) =>
    /^\d{4}-\d{2}-\d{2}$/.test(value) || "Неправильная дата выдачи паспорта.",
];
...
<template>
  <v-sheet v-if="isShow" class="mx-auto" maxWidth="300">
    <v-form
      ref="formRef"
      v-model="isValid"
    ...
    >
      ...
      <div v-if="!isNew">
        <v-btn @click="deleteEmp"> Удалить </v-btn>
        <v-btn v-if="isChange" type="submit"> Изменить </v-btn>
      </div>
      <v-btn v-else type="submit"> Добавить </v-btn>
    </v-form>
  </v-sheet>
</template>
